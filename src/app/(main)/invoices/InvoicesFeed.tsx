...
import ShowExpiredToggle, {ShowExpiredToggleSkeleton,} from "@/components/ShowExpiredToggle";
...
import {InvoiceProps, TicketStatusTypes, TotalPagesCountProps,} from "@/lib/types";
import {capitalizeSentences, formattedDate, getTimeLeftFromTimestamp,} from "@/lib/utils";
                        ...
                        <ScrollArea className="h-[42vh] space-y-1 py-1 w-full pr-1">
                            <div className="flex flex-col py-3 mb-2 gap-y-2">
                                {isPending || isFetching ? (
                                    ...
                                    <div
                                        className="text-destructive w-full text-center text-sm font-medium border border-destructive rounded-md p-4 bg-destructive/10">
                                        Cannot connect to server. Please try again later.
                                    </div>
                                ) : invoices.length === 0 ? (
                                    <div
                                        className="text-muted-foreground text-center w-full text-sm font-medium border border-border rounded-md p-4 bg-muted">
                                        No invoice found.
                                    </div>
                                ) : (
...
function InvoiceDetailsCard({invoice}: { invoice: InvoiceProps | null }) {
    if (!invoice) {
        return (
            <div
                className="flex mx-auto w-[300px] aspect-square shadow flex-col items-center justify-center rounded-xl border border-muted-foreground bg-muted p-6 text-muted-foreground space-y-4">
        ...
        <div className="flex w-full flex-col justify-center rounded-xl border bg-card p-6 shadow-sm space-y-4">
            <div className="w-full flex justify-between items-center">
                ...
                <Button className="text-primary" variant={"outline"} asChild>
                    <Link href={`/invoices/${invoice.TicketNumber}`}>See invoice</Link>
                </Button>
            </div>
...
function InvoiceDetailsDialog({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger>
                <div
                    className="cursor-pointer flex lg:hidden justify-center items-center rounded-lg border bg-card p-4 shadow hover:bg-accent hover:text-accent-foreground transition">
                    <div className="flex w-full justify-between items-start">
