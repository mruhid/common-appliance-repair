...
import {CCSTaffProps, EmployeeProps, InvoiceProps, TotalPagesCountProps} from "./types";
...
export async function findInvoiceByTicketNumber(
    number: string
): Promise<InvoiceProps | null> {
    if (!number.trim()) return null;

    const employeeRef = collection(db, "Jobs");
    const q = query(employeeRef, where("TicketNumber", "==", number));
    const snapshot = await getDocs(q);

    if (snapshot.empty) return null;

    return snapshot.docs[0].data() as InvoiceProps;
}
